...
def show_vote_history():
    ...
    history_window.title("Vote History")
    ...
    for i, block in enumerate(blockchain.chain):
        for vote in block.votes:
            listbox.insert(tk.END, f"Block {i} - Candidate: {vote['candidate']}")
...
def show_results_table():
    ...
    result_window.title("Results Table")

    tk.Label(result_window, text="Candidate", width=20, anchor='w', font=('Arial', 12, 'bold')).grid(row=0, column=0)
    tk.Label(result_window, text="Votes", width=10, anchor='w', font=('Arial', 12, 'bold')).grid(row=0, column=1)

    for i, (candidate, count) in enumerate(results.items(), start=1):
        tk.Label(result_window, text=candidate, anchor='w', width=20).grid(row=i, column=0)
        tk.Label(result_window, text=str(count), anchor='w', width=10).grid(row=i, column=1)

def show_pending_votes():
    if not blockchain.pending_votes:
        messagebox.showinfo("Pending Votes", "No pending votes.")
        return
    pending_window = tk.Toplevel(root)
    pending_window.title("Pending Votes")
    pending_window.geometry("350x250")

    tk.Label(pending_window, text="Pending Votes:", font=("Arial", 12, "bold")).pack()
    listbox = tk.Listbox(pending_window, width=50)
    for vote in blockchain.pending_votes:
        listbox.insert(tk.END, f"Candidate: {vote['candidate']}")
...
def export_blockchain():
    ...
    messagebox.showinfo("Export Complete", "Blockchain data exported to blockchain_export.json.")

def search_candidate():
    def do_search():
        name = entry_search.get()
        if not name:
            messagebox.showerror("Input Error", "Please enter a candidate name.")
            return
        count = sum(vote["candidate"] == name for block in blockchain.chain for vote in block.votes)
        messagebox.showinfo("Search Results", f"{name} has {count} vote(s).")

    search_window = tk.Toplevel(root)
    search_window.title("Search Candidate Votes")
    tk.Label(search_window, text="Enter Candidate Name:").pack()
...
tk.Button(root, text="Show Results (Table)", command=show_results_table).pack(pady=5)
tk.Button(root, text="View Vote History", command=show_vote_history).pack(pady=5)
tk.Button(root, text="View Pending Votes", command=show_pending_votes).pack(pady=5)
tk.Button(root, text="Search Candidate Votes", command=search_candidate).pack(pady=5)
tk.Button(root, text="Export Blockchain", command=export_blockchain).pack(pady=5)
